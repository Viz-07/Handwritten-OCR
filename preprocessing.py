"""
preprocessing.py — Stage 1: Preprocessing Engine
=================================================
...
def deskew(img: Image.Image) -> Image.Image:
    """Correct rotation using Hough transform on binarized edges."""
    ...
    if abs(median_angle) < 15:  # only correct small skews
        rotated = rotate(gray, median_angle, cval=255, preserve_range=True).astype(np.uint8)
...
def preprocess(
    ...
    *,
    ...
    """
    logger.info("Preprocessing: step 1/2 — grayscale")
    gray_img = to_grayscale(img)

    logger.info("Preprocessing: step 2/2 — Sauvola binarization")
    binary_img = sauvola_binarize(gray_img, window_size=sauvola_window, k=sauvola_k)
